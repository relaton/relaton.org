...
`TYPE`: optional. Specifies the standards class library to be used, that the identifier is part of. The recognized values for `TYPE` are: `BIPM`, `BSI`, `CC` (CalConnect), `CEN`, `CIE`, `CN` (Chinese GB standards), `ECMA`, `IEC`, `IEEE`, `IETF`, `IHO`, `ISO`, `ITU`, `NIST`, `OGC`, `OMG`, `UN`, `W3C`.
...
A Relaton YAML file describing an individual bibliographic entry is limited to metadata specific to that entry. Flavor gems have additional fields. The [Relaton YAML](https://github.com/relaton/relaton-bib/blob/master/docs/hash.adoc#yaml) illustrates the common fields supported by all flavor gems.
