...
This form of AsciiBib utilizes a definition list of element name and element contents,
...
title should be given in the content body:
...
The nested definition list syntax requires frequent breakouts into open blocks, to deal
with the limitation on AsciiDoc nested definition lists.

AsciiBib offers a "`Path style definition list`" syntax to
...
Whenever part of the key is repeated between entries, the entries are assumed to attach to the same parent. If an array of objects is needed, a blank entry is required for the key of each repeating element: For example,
