...
It is a subclass of the `nametemplate` (<<nametemplate>>), configured for
...
+
====
The size of a book may be expressed as _Vol. 3, pp. 9–20_, while the size of a
_xlii + 76 pp._
====
...
Description::
Creator names formatted according to the <<nametemplate>> each. The joining template
...
Description::
Creator names formatted according to the <<authorcitetemplate>> for citations.
...
Description::
Series information formatted according to the <<seriestemplate>>.
...
source will also need to be transformed. 
